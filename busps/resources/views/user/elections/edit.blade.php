...
<div class="col-md-12">
    <div class="card">
        <div class="header">
            <h4 class="title">Edit Election: {{ $election->election_name }}</h4>
            ...
            <form method="POST" action="{{ route('admin.elections.update', $election->election_id) }}">
                    ...
                    <select class="form-control" id="society_id" name="society_id" required>
                        @foreach($societies as $society)
                            <option value="{{ $society->society_id }}" 
                                {{ $election->society_id == $society->society_id ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <input type="number" class="form-control" id="election_year" name="election_year" 
                           min="1900" max="{{ date('Y') + 1 }}" value="{{ $election->election_year }}" required>
                </div>
